...
<task type="auto">
  ...
  <action>
    ...
    2. Parse specialist fields that appear as separate lines within task boundaries
    3. Store specialist assignments in an array indexed by task number
    4. Pattern to extract: lines matching `^specialist:` between `<task` and `</task>` markers
    ...
    while IFS= read -r line; do
      ...
      if [[ "$line" =~ ^\<task ]]; then
        TASK_NUM=$((TASK_NUM + 1))
        IN_TASK=true
        SPECIALISTS[$TASK_NUM]="gsd-executor"  # Default
      fi
    ...
    Alternative approach using awk for cleaner parsing:
    ```bash
    # Use awk to extract specialist for each task
    SPECIALISTS=($(awk '
      /<task/ { task++ }
      /^specialist:/ && task { print $2 }
      /<\/task>/ && !found_specialist[task] { print "gsd-executor"; found_specialist[task]=1 }
      /^specialist:/ { found_specialist[task]=1 }
    ' "$PLAN_FILE"))
    ```
