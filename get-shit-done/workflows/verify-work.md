...
**4.5b. Create CONTEXT.md for each fix phase (CRITICAL for autonomous flow):**

This step is REQUIRED for fix phases to flow autonomously through the it-just-works loop.
By creating CONTEXT.md, the fix phase appears as "discussed" status, skipping discuss-phase
and routing directly to plan-phase.
...
**Mode:** Fix phase (auto-generated from verification)
**Generated:** ${TIMESTAMP}

## Phase Boundary

Close gaps identified in Phase ${phase_number} verification. This is a targeted fix phase,
not exploratory work. The gaps are well-defined from verification analysis.

## Gaps to Close

EOF

# Append gap details from GAPS.md (skip frontmatter)
tail -n +8 "${directory}/GAPS.md" >> "$CONTEXT_PATH"
...
- Research skipped - gaps are well-defined from verification
- Focus on closing the specific gaps listed above
...
*Fix phase auto-generated by verify-work*
...
- [ ] Fix phases have both GAPS.md AND CONTEXT.md (for autonomous flow)
- [ ] CONTEXT.md has skip_research: true and fix_phase: true in frontmatter
